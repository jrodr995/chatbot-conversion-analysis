...
def _render_sql(cfg) -> str:
    ...
    return template.replace("{{DB}}", db).replace("{{SCHEMA}}", schema).replace("{{TABLE}}", table)
...
def _generate_synthetic_sample(num_rows: int = 800, seed: int | None = None) -> pd.DataFrame:
    ...
    # Baseline rates used for non-explicit requests (kept for documentation)
    ...
    for i in range(num_rows):
        ...
        appt_logit = (
            -2.0 + 0.06 * total_user_messages + 0.001 * duration + 0.04 * total_agent_messages
        )
        ...
        rfi_logit = (
            -0.2
            + 0.09 * (total_user_messages > 8)
            + 0.06 * (max_user_words > 12)
            + 0.001 * duration
        )
